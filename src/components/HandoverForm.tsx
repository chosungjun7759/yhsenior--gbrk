...
interface HandoverFormProps {
  userName: string;         // 인계자 (본인 자동)
  startDate: string;        // 휴가 시작일 (인계기간 기본값)
  endDate: string;          // 휴가 종료일
  initialData?: {
    items: HandoverItem[];
    date: string;
    period: string;
...
export default function HandoverForm({
  ...
  const notify = (
    newItems = items, newDate = date,
    newFrom = periodFrom, newTo2 = periodTo,
    newTo = to, newConfirmer = confirmer
  ) => {
    onChange({
  ...
  const updateItem = (id: string, field: keyof HandoverItem, value: string) => {
    ...
    setItems(updated);
    notify(updated, date, periodFrom, periodTo);
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input
            ...
            onChange={e => { setDate(e.target.value); notify(items, e.target.value); }}
            className={inputCls}
          />
        </div>
        <div>
          ...
          <input
            ...
            onChange={e => { setPeriodFrom(e.target.value); notify(items, date, e.target.value, periodTo); }}
            className={inputCls}
          />
        </div>
        <div>
          ...
          <input
            ...
            onChange={e => { setPeriodTo(e.target.value); notify(items, date, periodFrom, e.target.value); }}
            className={inputCls}
          />
        </div>
        <div>
        ...
        <div>
          ...
          <input
            type="text" value={to}
            onChange={e => { setTo(e.target.value); notify(items, date, periodFrom, periodTo, e.target.value); }}
            placeholder="인수자 성명"
            className={inputCls}
          />
        </div>
      ...
      <p className="text-[11px] text-slate-400">
        ※ 업무명과 주요 내용은 담당 업무에 맞게 직접 작성해 주세요.
      </p>
    </div>
  );
}
