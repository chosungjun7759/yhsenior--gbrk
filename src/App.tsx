...
export default function App() {
  ...
  useEffect(() => {
    const unsubAuth = onAuthStateChanged(auth, async (firebaseUser) => {
      setAuthReady(true);
      if (firebaseUser) {
        // 이메일로 Firestore users에서 해당 직원 찾기
        const allUsers = await dbService.getUsers();
        const matched = allUsers.find(u => u.email === firebaseUser.email);
        if (matched) {
          setCurrentUser(matched);
          if (matched.role === Role.DIRECTOR || matched.role === Role.MANAGER) {
  ...
  // Firestore 실시간 구독
  useEffect(() => {
    const unsubUsers = dbService.subscribeUsers(u => setUsers(u));
    const unsubReqs = dbService.subscribeRequests(r => setRequests(r));
  ...
  if (!currentUser) {
    return (
      <LoginScreen onLogin={() => showToast("로그인되었습니다.", "info")} />
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 pb-20 print:bg-white print:p-0">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8 print:hidden">
        {toast && (
        ...
        {isAdmin ? (
          <div className="space-y-6">
            {/* PC: 사이드바 + 메인 2컬럼 / 모바일: 단일 컬럼 */}
            <div className="flex flex-col lg:flex-row gap-6">

              {/* 사이드바 — PC에서만 표시 */}
              <div className="lg:w-56 flex-shrink-0">
                {/* 모바일: 가로 2버튼 / PC: 세로 사이드바 */}
                <div className="grid grid-cols-2 lg:grid-cols-1 gap-2">
                  {(() => {
                    const menus = [
                      { key:"APPLY",    icon:<Plus        className={`h-6 w-6 ${adminTab==="APPLY"?"text-white":"text-blue-500"}`} />,    label:"휴가 신청", sub:"신청서 작성",   badge:undefined },
                      { key:"APPROVAL", icon:<CheckSquare className={`h-6 w-6 ${adminTab==="APPROVAL"?"text-white":"text-amber-500"}`} />, label:"결재 대기", sub:"승인 처리함", badge:pendingCount },
                    ];
                    return menus.map(({ key, icon, label, sub, badge }) => (
                      <button key={key} onClick={() => setAdminTab(key as AdminTab)}
                        className={`flex lg:flex-row flex-col items-center lg:items-start justify-center lg:justify-start p-4 lg:px-4 lg:py-3.5 rounded-2xl border transition-all text-center lg:text-left gap-1.5 lg:gap-3 shadow-sm cursor-pointer ${
                          adminTab === key ? "bg-blue-600 border-blue-600 text-white" : "bg-white border-slate-200 text-slate-700 hover:border-blue-300 hover:bg-slate-50"
                        }`}>
                        {icon}
                        <div>
                          <span className="text-sm font-bold flex items-center gap-1.5">
                            {label}
                            {badge && badge > 0 && (
                              <span className="bg-rose-500 text-white text-[10px] font-black w-5 h-5 rounded-full flex items-center justify-center animate-pulse">{badge}</span>
                            )}
                          </span>
                          <span className={`text-[11px] hidden lg:block ${adminTab===key?"text-blue-100":"text-slate-400"}`}>{sub}</span>
                        </div>
                      </button>
                    ));
                  })()}
                  ...
                  {/* PC 사이드바 하단: 내 잔여연차 */}
                  <div className="hidden lg:block bg-white border border-slate-200 rounded-2xl px-4 py-4 shadow-sm mt-2">
                    <p className="text-[11px] font-bold text-slate-400 mb-1">내 잔여 연차</p>
                    <p className="text-2xl font-black text-blue-600 font-mono">{myRemainingLeave}일</p>
                    {currentUser.role === Role.DIRECTOR && (
                      <div className="mt-3 space-y-1.5">
                        <button onClick={() => setShowDashboard(d => !d)}
                          className={`w-full flex items-center gap-1.5 text-xs border px-2.5 py-1.5 rounded-lg transition-colors cursor-pointer font-bold ${showDashboard ? "bg-emerald-600 border-emerald-600 text-white" : "text-emerald-600 border-emerald-200 bg-emerald-50 hover:bg-emerald-100"}`}>
                          <Users className="h-3.5 w-3.5" />
                          {showDashboard ? "현황 닫기" : "연차 현황"}
                        </button>
                        <button onClick={() => setShowLeaveReset(true)}
                          className="w-full flex items-center gap-1.5 text-xs text-amber-600 border border-amber-200 bg-amber-50 hover:bg-amber-100 px-2.5 py-1.5 rounded-lg transition-colors cursor-pointer font-bold">
              ...
              <div className="flex-1 min-w-0 space-y-4">
                <div className="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 shadow-sm">
                  {adminTab === "APPLY" && (
                    <div className="space-y-4">
                      ...
                      <LeaveRequestForm currentUser={currentUser} remainingLeave={myRemainingLeave}
                  ...
                  {(adminTab === "STATUS" || adminTab === "HISTORY") && !showDashboard && (
                    <div className="text-center py-10 text-slate-400">
                      <p className="text-sm font-bold">아래 연차 현황 버튼을 클릭해주세요.</p>
                    </div>
                  )}
                </div>

                {/* 모바일 전용: 잔여연차 + 버튼 */}
                <div className="lg:hidden bg-white border border-slate-200 rounded-2xl px-5 py-4 flex items-center justify-between shadow-sm">
                  ...
                  <div className="flex items-center gap-2">
                    {currentUser.role === Role.DIRECTOR && (
                      <>
                        ...
                        <button onClick={() => setShowLeaveReset(true)}
                          className="flex items-center gap-1.5 text-xs text-amber-600 border border-amber-200 bg-amber-50 hover:bg-amber-100 px-3 py-1.5 rounded-lg transition-colors cursor-pointer font-bold">
                          <RefreshCw className="h-3.5 w-3.5" />
                          초기화
                        </button>
                      </>
                ...
                {showDashboard && currentUser.role === Role.DIRECTOR && (
                  <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
                    ...
                    <div className="p-6">
                      {adminTab === "STATUS" && <DirectorDashboard users={users} requests={requests} onlyStatus />}
                      {adminTab === "HISTORY" && (
                        <div className="space-y-8">
                          ...
                          <div className="border-t pt-6 space-y-4">
                            ...
                            <RequestHistory currentUser={currentUser} requests={requests}
          ...
          <div className="space-y-6 max-w-2xl mx-auto">
            ...
            {!isApplyingFormOpen ? (
              <button onClick={() => setIsApplyingFormOpen(true)}
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-5">
                <div className="flex items-center justify-between border-b pb-4">
                  ...
                  <button onClick={() => setIsApplyingFormOpen(false)}
